...
def extract_features(model, data_loader, choice_c=0, adaibn=False, print_freq=100, metric=None):
    ...
    with torch.no_grad():
        for i, item in enumerate(data_loader): 
            imgs, pids, cams =item[0], item[1], item[choice_c+2]
            # print(item)
            # print("IMG")
            # print(imgs)
            # print("FNAME")
            ...
            for img, output, pid, cam in zip(imgs, outputs, pids, cams):
                features[img] = output
                labels[img] = pid
                cam_angles[img] = cam
    ...
    return features, labels, cam_angles, images

def pairwise_distance(features= None, query=None, gallery=None, metric=None):
    ...
    dist_m = torch.pow(x, 2).sum(dim=1, keepdim=True) + \
             torch.pow(y, 2).sum(dim=1, keepdim=True).t()
    dist_m -= 2 * torch.mm(x, y.t())
